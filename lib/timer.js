...
export function blankSession(rate, currencyCode, ratePeriod, periodSeconds) {
  return {
    startTimestamp: null,
    accumulatedMs: 0,
    paused: true,
...
/** Raw earnings value given session (currency-agnostic) */
export function calculateEarnings(session) {
  const { rate, ratePeriod } = session;
  const seconds = elapsedSeconds(session);
  // import avoided here — caller uses rates.js computeEarnings
  // fallback: treat as hourly
  const periodSeconds = session.periodSeconds ?? 3600;
  const perSecond = rate / periodSeconds;
